...
export const I18N = {
  app_name: {
    ...
    zh_TW: `序列翻譯`,
    ja: `Sequence Translator`,
    ko: `Sequence Translator`,
  },
  translate: {
    zh: `翻译`,
    en: `Translate`,
  ...
  rules_warn_1: {
    ...
    zh_TW: `1.規則生效的優先順序依序為：個人規則 > 全域規則。 "全域規則"相當於兜底規則。`,
    ja: `1. ルールの優先順位: 個人ルール > グローバルルール。「グローバルルール」はフォールバックルールのようなものです。`,
    ko: `1. 규칙 우선순위: 개인 규칙 > 전역 규칙. "전역 규칙"은 일종의 폴백(fallback) 규칙입니다.`,
  },
  rules_warn_2: {
    ...
    zh_TW: `2. 規則填寫說明：輸入框留空或下拉選擇「*」表示使用全域規則。CSS 選擇器支援使用前綴：「+」表示在全域規則基礎上追加，「-」表示剔除。`,
    ja: `2. ルールの記入について: 入力ボックスを空白にするか、ドロップダウンで「*」を選択すると、グローバルルールが使用されます。CSS セレクターはプレフィックスに対応しています。「+」はグローバルルールへの追加、「-」は除外を意味します。`,
    ko: `2. 규칙 작성 관련: 입력란을 비워두거나 드롭다운에서 "*"를 선택하면 전역 규칙이 사용됩니다. CSS 선택자는 접두사를 지원합니다. "+"는 전역 규칙에 추가, "-"는 제외를 의미합니다.`,
  },
  sync_warn: {
  ...
  reg_niutrans: {
    ...
    zh_TW: `取得小牛翻譯金鑰【序列翻譯專屬新用戶註冊贈送 300 萬字元】`,
    ja: `NiuTrans APIキーを取得 [Sequence翻訳 専用 新規ユーザー登録で300万文字無料]`,
    ko: `NiuTrans API 키 받기 [Sequence 번역기 신규 사용자 등록 시 300만 자 무료 제공]`,
  },
  trigger_mode: {
    zh: `触发方式`,
    en: `Trigger Mode`,
